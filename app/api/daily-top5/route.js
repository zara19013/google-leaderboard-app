...
async function bqQuery(token, query) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ query, useLegacySql: false, timeoutMs: 55000, maxResults: 100 }),
  });
...
function tableForDate(date) {
  const month = date.substring(0, 7);
  return MONTH_TABLE_MAP[month] || null;
}

function buildDailyQuery(date) {
  const table = tableForDate(date);
  if (!table) throw new Error(`No Google spend table for ${date.substring(0, 7)}`);

  return `
WITH day_spend AS (
  SELECT ad_name, SUM(total_spend) AS day_spend
  ...
  SELECT
    ...
    CASE
      ...
      ELSE COALESCE(
        ...
        REGEXP_EXTRACT(d.ad_name, r'(?i)(?:^|[_ +\\-])(Liran|Farwa|Damia|Isaac|Iason|Tristan|Roman|Julija|Jaouad|Kevin|Owen|Thomas|Lauris|Zayd|Sam|Cosmin|Fotis|Marco|Emilis|Johnny|Lucas|Lukas|Airidas|Neil|Kandy|Adrian|David|Jamil|Mia|Martin|Will|Nina|Oliver|Brian|Aisha|Ezra|Dex|Georgio|Bilal)(?:[_ +\\-]|$)'),
        CASE
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD1[^0-9]')  THEN 'Adil Amarsi'
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD2[^0-9]')  THEN 'Adrian'
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD3[^0-9]')  THEN 'Airidas'
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD25')        THEN 'Liran / Farwa'
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD26')        THEN 'Liran / Farwa'
          WHEN REGEXP_CONTAINS(d.ad_name, r'(?i)POD27')        THEN 'Lucas'
        END
      )
    END AS strategist
  FROM day_spend d
...
SELECT
  ...
  COUNT(DISTINCT ad_name)            AS ads_running
FROM attr
...
export async function GET(request) {
  ...
  let dailyQuery, totalQuery;
  try {
    dailyQuery = buildDailyQuery(date);
    totalQuery = buildTotalQuery(date);
  } catch (err) {
  ...
  if (process.env.VERCEL) {
    try {
      ...
      const [top5, totals] = await Promise.all([
        bqQuery(token, dailyQuery),
